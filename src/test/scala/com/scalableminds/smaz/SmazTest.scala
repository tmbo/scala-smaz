...
import com.scalableminds.smaz.Smaz
...
class SmazTest extends Specification {
  ...
  "Smaz compressor" should {
    "compress and decompress simple string" in {
      ...
      val compressed = Smaz.compress(str)
      val uncompressed = Smaz.decompress(compressed)
    ...
    "run original smaz tests" in {
      ...
      val compressed = strings.map(s => Smaz.compress(s))
      val uncompressed = compressed.map(c => Smaz.decompress(c))
