...
trait DefaultCodeBook{
  ...
  // is as follows: "\length<str>\replacement". Where length and replacement are 
  // expressed as octal numbers. The whole array is used a hashmap. 
  val CODEBOOK = Array(
  ...
  // The reverse codebook will be used during decompression. When an encoded byte 
  // is encountered its value is used as an index into this array to look up the 
  // replacement.
  val REVERSE_CODEBOOK = Array(
